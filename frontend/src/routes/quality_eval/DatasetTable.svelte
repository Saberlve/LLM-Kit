<script lang="ts">
  ...
  // Use English column names to match construct interface
  ...
  async function delete_handle() {
    ...
    try {
      ...
      const response = await axios.delete(`http://127.0.0.1:8000/quality/delete_file/${id_to_delete}`);
  ...
  async function previewDataset(dataset) {
    ...
    try {
      const response = await axios.get(`http://127.0.0.1:8000/quality/preview/${dataset.id}?page=${previewCurrentPage}&page_size=${previewItemsPerPage}`);
  ...
  async function goToPage(page) {
    if (page >= 1 && page <= previewTotalPages) {
      ...
      try {
        ...
        const response = await axios.get(`http://127.0.0.1:8000/quality/preview/${dataset.id}?page=${page}&page_size=${previewItemsPerPage}`);
        if (response.status === 200) {
          previewContent = response.data.items || [];
          previewTotalPages = response.data.total_pages || 1;
        } else {
  ...
  function downloadDataset(dataset) {
    window.open(`http://127.0.0.1:8000/quality/download/${dataset.id}`, '_blank');
  }
</script>
      ...
      <div class="bg-white rounded-lg overflow-hidden mb-4">
        <Table striped={true}>
          <TableHead>
            ...
            <TableHeadCell>Quality Scores</TableHeadCell>
          </TableHead>
          <TableBody>
